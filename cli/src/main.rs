...
enum Commands {
    ...
    /// analysing
    Analysis {
        /// Sets vertex file folder path which is generated by 'Data Clean' command
        #[arg(short, long, value_name = "VERTEXES")]
        vertex: Vec<PathBuf>,
...
fn main() {
    ...
    match &cli.command {
        ...
        Some(Commands::Analysis { vertex: vertexes, output, exclude: excludes }) => {
            let mut validated_vertexes: Vec<PathBuf> = Vec::new();
            for vertex in vertexes {
                match fs::canonicalize(vertex) {
                    Ok(canonicalize_path) => validated_vertexes.push(canonicalize_path),
                    Err(err) => {
                        let mut cmd = Cli::command();
                        cmd.error(ErrorKind::InvalidValue, 
                            format!("{}: {}", err, vertex.to_str().unwrap())).exit();
                    }
                }
            }
            ...
            match suzaku_core::analysis(&validated_vertexes, &output_dir.unwrap(), &excluded_paths) {
