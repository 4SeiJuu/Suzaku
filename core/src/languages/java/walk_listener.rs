...
pub struct WalkListener {
    stack: Stack,
}

impl WalkListener {
    pub fn new() -> Self {
        WalkListener { stack: Stack::new() }
    }
        ...
        match self.stack.pop() {
            Some(mut node) => {
                ...
                match self.stack.top_mut() {
                    Some(parent) => {
                        parent.get_members_mut().push_back(node);
                    },
...
impl<'input, 'a, Node: ParserNodeType<'input>> ParseTreeListener<'input, Node> for WalkListener {
...
impl<'input> JavaParserListener<'input> for WalkListener {
    /**
    ...
    fn enter_typeDeclaration(&mut self, _ctx: &TypeDeclarationContext<'input>) {
        println!("enter_typeDeclaration: {:?}", _ctx.get_text());

        // fn walker(child_strs: &mut Vec::<String>, iter: Box<dyn Iterator<Item = Rc<dyn JavaParserContext>>>) {
        //     for child in iter {
        //         let child_vs_type = (child as Rc<dyn JavaParserContext>);
        //         child_strs.push(child_vs_type.get_text());
        //         if child_vs_type.get_child_count() > 0 {
        //             walker(child_strs, child_vs_type.get_children());
        //         }
        //     }
        // }

        // let children = _ctx.get_children();
        // let mut child_strs: Vec<String> = Vec::<String>::new();
        // walker(&mut child_strs, children);
        // println!("CHILDREN: {}", child_strs.join("|"));

        // let mut child_strs = Vec::new();
        // let mut child_tree_walker = |iter| {
        //     for child in iter {
        //         let child_vs_type = (child as Rc<dyn JavaParserContext>);
        //         child_strs.push(child_vs_type.get_text());
        //         if child_vs_type.get_child_count() > 0 {
        //             self(child_vs_type.get_children());
        //         }
        //     }
        // };
        // child_tree_walker(_ctx.get_children());
        // println!("Child Count: {}, Children: {}", _ctx.get_child_count(), child_strs.join("|"));
    }
    /**
    ...
    fn exit_typeDeclaration(&mut self, _ctx: &TypeDeclarationContext<'input>) {
        println!("exit_typeDeclaration: {:?}", _ctx.get_text());
    }
    /**
    ...
    fn exit_typeType(&mut self, _ctx: &TypeTypeContext<'input>) {
        println!("exit_typeType: {:?}", _ctx.get_child_count());
    }
    /**
