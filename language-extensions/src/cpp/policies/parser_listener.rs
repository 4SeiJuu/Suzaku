use antlr_rust::{tree::{ParseTreeListener, TerminalNode, ErrorNode}, parser::ParserNodeType};
use suzaku_extension_sdk::parser_listener::ParserListener;

use crate::cpp::{
    data::meta_type::MetaType,
    generated::{
        cpp14parserlistener::CPP14ParserListener,
        cpp14parser::*
    }
};

impl<'input> ParseTreeListener<'input, CPP14ParserContextType> for ParserListener<MetaType> {
    /// Called when parser creates terminal node
    fn visit_terminal(&mut self, _node: &TerminalNode<'input, CPP14ParserContextType>) {}
    /// Called when parser creates error node
    fn visit_error_node(&mut self, _node: &ErrorNode<'input, CPP14ParserContextType>) {}
    /// Called when parser enters any rule node
    fn enter_every_rule(&mut self, _ctx: &<CPP14ParserContextType as ParserNodeType>::Type) {}
    /// Called when parser exits any rule node
    fn exit_every_rule(&mut self, _ctx: &<CPP14ParserContextType as ParserNodeType>::Type) {}
}


impl<'input> CPP14ParserListener<'input> for ParserListener<MetaType> {
    /**
